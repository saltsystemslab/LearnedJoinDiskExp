...
static const char *FLAGS_file_names = "";
...
int main(int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    else if (is_flag(argv[i], "--file_names="))
    {
      FLAGS_file_names = argv[i] + strlen("--file_names=");
    }
    else
    {
  ...
  if (FLAGS_file_names != "")
  {
    stringstream s(FLAGS_file_names);
    while (s.good())
    {
      string substr;
      getline(s, substr, ',');
      ...
      if (new_file.is_open())
      {
        ...
        num_keys.push_back(number_of_lines);
      }
    }
  }
  else
  ...
  int num_of_lists = FLAGS_num_of_lists;
  ...
  for (int i = 0; i < num_of_lists; i++)
  {
    ...
    {
      fstream new_file;
      new_file.open(filenames[i], ios::in);
      if (new_file.is_open())
      {
        string s;
        int number_of_lines = 0;

...
int main(int argc, char **argv) {
  ...
  if (FLAGS_disk_backed) {
    ...
    builder = new SliceArrayWithModelBuilder(stoi(FLAGS_num_keys),
...
#if LEARNED_MERGE
    bool status =
        LearnedLookup::lookup(iterator, stoi(FLAGS_num_keys), c, query);
    // if(status == true) {
...
#else
    bool status =
        StandardLookup::lookup(iterator, stoi(FLAGS_num_keys), c, query);
    // if(status == true) {
    //     assert(std::find(keys.begin(), keys.end(), queries[i]) !=
    //     keys.end());
    // }

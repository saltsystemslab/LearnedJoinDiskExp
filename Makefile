...
LEARNED_MERGE= -DUSE_LONG_DOUBLE=${USE_LONG_DOUBLE} -DTRACK_STATS=${TRACK_STATS} -DTRACK_PLR_TRAIN_TIME=0 -DTRUST_ERROR_BOUNDS=${TRUST_ERROR_BOUNDS} -DPLR_POS_OFFSET=${ERROR_BOUND} -DPGM_ERROR_BOUND=1 -DUSE_STRING_KEYS=${USE_STRING_KEYS} -DUSE_INT_128=${USE_INT_128} -DUSE_BULK_COPY=${USE_BULK_COPY} -DMAX_KEYS_TO_BULK_COPY=1000000 -DTRAIN_RESULT=${TRAIN_RESULT}

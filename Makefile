...
LEARNED_MERGE=-DUSE_PLR_MODEL=${USE_PLR_MODEL} -DUSE_PGM_INDEX=${USE_PGM_INDEX} -DUSE_BINARY_SEARCH=${USE_BINARY_SEARCH} -DLEARNED_MERGE=${USE_LEARNED_MERGE} -DTRACK_STATS=0 -DTRACK_BINARY_SEARCH_STATS=0 -DTRACK_PLR_TRAIN_TIME=0 -DTRUST_ERROR_BOUNDS=${TRUST_ERROR_BOUNDS} -DASSERT_SORT=0 -DPLR_POS_OFFSET=${ERROR_BOUND} -DPLR_ERROR_BOUND=$(ERROR_BOUND) -DPGM_ERROR_BOUND=$(ERROR_BOUND) -DUSE_STRING_KEYS=${USE_STRING_KEYS} -DUSE_INT_128=${USE_INT_128} -DUSE_BULK_COPY=${USE_BULK_COPY} -DMAX_KEYS_TO_BULK_COPY=1000000 -DTRAIN_RESULT=${TRAIN_RESULT}


OBJDIR=obj
_OBJ=slice_file_iterator.o plr.o plr_model.o pgm_index.o binary_search.o

    ...
    class node : public serializable {
       public:
        ...
        typename pivot_map::iterator get_pivot(const Key &k) {
            return get_pivot<typename pivot_map::iterator, pivot_map>(pivots,
                                                                      k);
        }
        ...
        typename message_map::const_iterator get_element_begin(
            const Key &k) const {
        ...
        typename message_map::iterator get_element_begin(
            const typename pivot_map::iterator it) {
            return it == pivots.end() ? elements.end()
                                      : get_element_begin(it->first);
        }
        ...
        void apply(const MessageKey<Key> &mkey, const Message<Value> &elt,
            ...
            switch (elt.opcode) {
                ...
                case DELETE:
                    elements.erase(elements.lower_bound(mkey.range_start()),
                                   elements.upper_bound(mkey.range_end()));
                    if (!is_leaf()) elements[mkey] = elt;
                    break;

                case UPDATE: {
                    auto iter = elements.upper_bound(mkey.range_end());
                    if (iter != elements.begin()) iter--;
        ...
        pivot_map split(betree &bet) {
            ...
            // nodes to have size between 0.4 * MAX_NODE_SIZE and 0.6 *
            // MAX_NODE_SIZE.
            int num_new_leaves = (pivots.size() + elements.size()) /
                                 (10 * bet.max_node_size / 24);
            ...
            for (int i = 0; i < num_new_leaves; i++) {
                ...
                result[pivot_idx != pivots.end() ? pivot_idx->first
                                                 : elt_idx->first.key] =
                    child_info(new_node, new_node->elements.size() +
                                             new_node->pivots.size());
                while (
                    things_moved < (i + 1) * things_per_new_leaf &&
                    ...
                    if (pivot_idx != pivots.end()) {
                        ...
                        while (elt_idx != elt_end) {
                            new_node->elements[elt_idx->first] =
                                elt_idx->second;
                            ++elt_idx;
                            things_moved++;
                        }
        ...
        void merge_small_children(betree &bet) {
            ...
            for (auto beginit = pivots.begin(); beginit != pivots.end();
                 ++beginit) {
                ...
                if (endit != beginit) {
                    ...
                    pivots[key] = child_info(merged_node,
                                             merged_node->pivots.size() +
        ...
        pivot_map flush(betree &bet, message_map &elts) {
            ...
            if (first_pivot_idx == last_pivot_idx &&
                ...
                while (elements.size() + pivots.size() >= bet.max_node_size) {
                    // Find the child with the largest set of messages in our
                    // buffer
                    ...
                    if (!(max_size > bet.min_flush_size ||
                        ...
                        break;  // We need to split because we have too many
                                // pivots
                ...
                // We have too many pivots to efficiently flush stuff down, so
                // split
                if (elements.size() + pivots.size() > bet.max_node_size) {
                    result = split(bet);
                }
        ...
        Value query(const betree &bet, const Key k) const {
            ...
            while (message_iter != elements.end() &&
                   message_iter->first.key == k) {
        ...
        get_next_message_from_children(const MessageKey<Key> *mkey) const {
            if (mkey && *mkey < pivots.begin()->first) mkey = NULL;
        ...
        std::pair<MessageKey<Key>, Message<Value> > get_next_message(
       ...
       public:
        ...
        void apply(const MessageKey<Key> &msgkey, const Message<Value> &msg) {
            switch (msg.opcode) {
                ...
                case UPDATE:
                    first = msgkey.key;
                    if (is_valid == false) second = bet.default_value;

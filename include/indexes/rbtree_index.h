...
public:
  RbTreeIndexBuilder(Comparator<KVSlice> *comp, int key_size_bytes) : num_elts_(0) {
    tree_ = new RbTree(RbTreeComparator(comp, key_size_bytes));
  }
  void add(const KVSlice &t) override {
    tree_->insert(RbTreeEntry(std::string(t.data(), t.key_size_bytes()), num_elts_));
    num_elts_++;
  }
  Index<KVSlice> *build() override {
    return new RbTreeIndex(tree_);
  }

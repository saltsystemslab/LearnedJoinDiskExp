...
typedef stx::btree_multimap<uint64_t, uint32_t, std::less<uint64_t>,
                            traits_inner>
    stx_btree;
...
public:
  BTreeWIndex(stx_btree *tree, int block_size, int num_blocks,
              uint64_t start_idx = 0, uint64_t end_idx = -1)
      : tree_(tree), block_size_(block_size), num_blocks_(num_blocks),
...
public:
  BTreeIndexBuilder() : num_elts_(0), block_id(0) { tree_ = new stx_btree(); }

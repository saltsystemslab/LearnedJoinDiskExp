...
public:
  ...
  uint64_t getApproxPositionMonotoneAccess(const T &t) override {
    ...
    if (cur_segment_index_) cur_segment_index_--;
    return std::min<size_t>((*segments_)[cur_segment_index_](point), (*segments_)[cur_segment_index_+1].intercept);
  };
...
private:
  ...
  std::vector<pgm::PGMIndex<POINT_FLOAT_TYPE, 64, 0, float>::Segment> *segments_;
  pgm::OneLevelPGMIndex<POINT_FLOAT_TYPE, Epsilon> *pgm_index_;
  KeyToPointConverter<T> *converter_;
};

template <class T, uint64_t Epsilon> class OneLevelPgmIndexBuilder : public IndexBuilder<T> {
public:
  OneLevelPgmIndexBuilder(uint64_t approx_num_keys, KeyToPointConverter<T> *converter)
  ...
  Index<KVSlice> *build() override {
    return new OneLevelPgmIndex<T, Epsilon>(new pgm::OneLevelPGMIndex<POINT_FLOAT_TYPE, Epsilon>(x_points_),
                           converter_);
  }

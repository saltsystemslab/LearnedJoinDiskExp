...
template <class T> class IntDiskIterator : public Iterator<T> {
public:
  IntDiskIterator(int file_descriptor, uint64_t num_keys, std::string id)
      : file_descriptor_(file_descriptor),
        ...
        peek_key_block_(
            new FileKeyBlock(file_descriptor, PAGE_SIZE, sizeof(T))),
        cur_idx_(0), id_(id), num_keys_(num_keys), start_key_idx_offest_(0) {}
  ...
  IntDiskIterator(int file_descriptor, uint64_t start_key_idx_offest,
      ...
      : file_descriptor_(file_descriptor),
        ...
        peek_key_block_(
            new FileKeyBlock(file_descriptor, PAGE_SIZE, sizeof(T))),
        cur_idx_(0), id_(id), num_keys_(num_keys),
  ...
  Iterator<T> *subRange(uint64_t start_key_idx, uint64_t end_key_idx) override {
    return new IntDiskIterator(file_descriptor_, start_key_idx,
                               end_key_idx - start_key_idx,
                               id_ + "[" + std::to_string(start_key_idx) + "," +
                                   std::to_string(end_key_idx) + "]");
  }
...
template <class T> class IntDiskBuilder : public IteratorBuilder<T> {
public:
  IntDiskBuilder(const char *file_name, size_t buffer_size, std::string id)
      : file_name_(new char[strlen(file_name) + 1]), buffer_size_(buffer_size),
        key_size_(sizeof(T)), num_keys_(0), buffer_idx_(0), id_(id),
        file_offset_(0), start_offset_byte_(0) {
  ...
  IntDiskBuilder(const char *file_name, size_t buffer_size,
      ...
      : file_name_(new char[strlen(file_name) + 1]), buffer_size_(buffer_size),
        key_size_(sizeof(T)), num_keys_(0), buffer_idx_(0), id_(id),
        file_offset_(0), start_offset_byte_(start_offset_byte) {
    memcpy(file_name_, file_name, strlen(file_name));
    file_name_[strlen(file_name)] = '\0';
    file_descriptor_ = open(file_name_, O_WRONLY | O_CREAT, 0644);
    if (file_descriptor_ == -1) {
  ...
  IteratorBuilder<T> *subRange(uint64_t start, uint64_t end) override {
    ...
    return new IntDiskBuilder(file_name_, buffer_size_, start * key_size_,
                              id_ + "[" + std::to_string(start) + "," +
                                  std::to_string(end) + "]");
  }

...
public:
  virtual SearchResult search(Window<T> window, T Key, Bounds bound,
                              Comparator<T> *c) = 0;
};

class LinearSearch : public SearchStrategy<KVSlice> {
public:
  SearchResult search(Window<KVSlice> window, KVSlice kv, Bounds bound,
                      Comparator<KVSlice> *c) override {
    if (bound.lower >= bound.upper) {
      return {bound.lower, false, false};
    }
    uint64_t i;
    bool found = false;
    for (i = 0; i < window.buf_len; i += kv.total_size_bytes()) {
      KVSlice candidate(window.buf + i, kv.key_size_bytes(),
                        kv.value_size_bytes());
      ...
      if (compResult == 0) {
        return {window.lo_idx + i / kv.total_size_bytes(), true, false};
      }
      if (compResult > 0) {
        return {window.lo_idx + i / kv.total_size_bytes(), false, false};
      }
    }
    return {window.lo_idx + i / kv.total_size_bytes(), false, true};
  }
};
...
private:
  ...
  uint64_t find_lower_bound(char *buf, uint64_t len_in_bytes, KVSlice kv,
                            Comparator<KVSlice> *c) {
    uint64_t start = 0;
    int kvSize = kv.total_size_bytes();
    uint64_t end = (len_in_bytes) / (kvSize);
    uint64_t len = end - start;
    uint64_t step = bit_floor(len);
    KVSlice cur1(buf + (kvSize) * (step + start), kv.key_size_bytes(),
                 kv.value_size_bytes());
    ...
    for (step /= 2; step != 0; step /= 2) {
      KVSlice cur(buf + (kvSize) * (step + start), kv.key_size_bytes(),
                  kv.value_size_bytes());
...
public:
  SearchResult search(Window<KVSlice> window, KVSlice kv, Bounds bound,
    ...
    int kvSize = kv.total_size_bytes();
    KVSlice last_key(window.buf + window.buf_len - kvSize, kv.key_size_bytes(),
                     kv.value_size_bytes());
    ...
    KVSlice lowerBoundKey(window.buf + lower_bound * kvSize,
                          kv.key_size_bytes(), kv.value_size_bytes());
...
private:
  void getBoundForward(char *buf, uint64_t sz, uint64_t key, uint64_t *bound_lo,
                       uint64_t *bound_hi) {
  ...
  void getBoundBackward(char *buf, uint64_t hi, uint64_t key,
                        uint64_t *bound_lo, uint64_t *bound_hi) {
    // Last value that is lesser than or equal to lo
    if (hi == -1) {
      abort();
    }
...
public:
  SearchResult search(Window<KVSlice> window, KVSlice kv, Bounds bound,
    ...
    uint64_t pos =
        std::clamp(bound.approx_pos, window.lo_idx, window.hi_idx - 1) -
        window.lo_idx;
    ...
    if (pos_key < key_value) {
      getBoundForward(window.buf + (pos * 16), (window.buf_len - pos * 16) / 16,
                      key_value, &lo, &hi);

...
template <class T> struct StandardMergerArgs {
...
template <class T> class ParallelStandardMerger {
public:
  static Iterator<T> *merge(Iterator<T> *iter1, Iterator<T> *iter2,
                            int num_threads, Comparator<T> *comparator,
                            IteratorBuilder<T> *result) {
...
private:
  static void *standard_merge(void *a) {
    ...
    StandardMerger<T>::merge(args->iterators, args->n, args->comparator,
                             args->result);
    return NULL;
  }
};

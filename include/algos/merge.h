...
template <class T> class StandardMerge : public BaseMergeAndJoinOp<T> {
public:
  StandardMerge(SSTable<T> *outer, SSTable<T> *inner, Index<T> *inner_index,
  ...
  void doOpOnPartition(Partition partition, TableOpResult<T> *result) override {
    ...
    while (outer_iter->currentPos() < outer_end) {
      while (inner_iter->currentPos() < inner_end &&
             this->comparator_->compare(inner_iter->key(), outer_iter->key()) <=
                 0) {
    ...
    result->stats["inner_total_bytes_fetched"] =
        inner_iter->getTotalBytesFetched();
    ...
    result->stats["outer_total_bytes_fetched"] =
        outer_iter->getTotalBytesFetched();

    result->output_table = result_builder->build(), delete outer_iter;
    delete inner_iter;
  }
};

template <class T> class LearnedMerge1Way : public BaseMergeAndJoinOp<T> {
public:
  LearnedMerge1Way(SSTable<T> *outer, SSTable<T> *inner, Index<T> *inner_index,
  ...
  void doOpOnPartition(Partition partition, TableOpResult<T> *result) override {
    ...
    while (outer_iter->currentPos() < outer_end) {
      auto bounds = inner_index->getPositionBounds(outer_iter->key());
      ...
      // The next key is greater in inner, so just copy the outer key and get
      // out.
      ...
      if (bounds.lower > next_inner_key_to_add) {
        auto window =
            inner_iter->getWindow(next_inner_key_to_add, bounds.lower);
      ...
      next_inner_key_to_add =
          result.lower_bound; // Never search before this again.
      result_builder->add(outer_iter->key());
      outer_iter->next();
    }

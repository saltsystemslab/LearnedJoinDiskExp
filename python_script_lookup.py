...
use_string=[0,1]
learned_lookup=[1, 2]
os.system("echo no_of_items,no_of_queries,use_disk,use_string_keys,use_int_128,error_bound,lookup_type,training_duration,segment_count,lookup_duration >> result1.txt")
                ...
                for k in use_128:
                    ...
                    for l in error:
                        for m in learned_lookup:
                                            ...
                                            if(s==0):
                                                use_string_keys = "no string keys"
                                                os.environ["USE_STRING_KEYS"] = str(0)
                                            elif(s==1):
                                                use_string_keys = "string keys"
                                                os.environ["USE_STRING_KEYS"] = str(1)

                                            os.system("echo -n "+str(a)+"," ">>result1.txt")
                                            os.system("echo -n "+str(b)+"," ">>result1.txt")
                                            os.system("echo -n "+use_disk+","+use_string_keys+","+use_128_bit+","+str(l)+","+lookup+",>> result1.txt")
                                            os.environ["USE_INT_128"] = str(k)
                                            os.environ["ERROR_BOUND"] = str(l)
                                            os.system("make clean benchmark_lookup")
                                            #dir='./bin/'

                                            result = subprocess.run(["./bin/benchmark_lookup", '--num_keys='+str(a), '--num_queries='+str(b), '--use_disk='+str(j)], capture_output=True, text=True)
